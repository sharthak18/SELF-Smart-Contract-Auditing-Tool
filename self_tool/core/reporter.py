...
Includes deterministic review guidance, suppression context, and protocol context.
"""
...
def generate_report(
    ...
    has_review = any(i.review_status for i in issues)
    sections = []
...
> Powered by hardcoded exploit-class detectors and deterministic review profiles.
{'> **Built-in Deep Review:** every finding includes a proof obligation and test recipe.' if has_review else ''}
...
""")
    ...
    # ── Findings Index ───────────────────────────────────────────────────────
    if active_issues:
        review_col = " | Review Status" if has_review else ""
        index_header = f"| # | Severity | ID | Title | Location{review_col} |"
        index_sep = f"|---|----------|----|-------|----------{' | ---' if has_review else ''} |"
        index_rows = []
        for i, issue in enumerate(active_issues, 1):
            review_cell = ""
            if has_review and issue.review_status:
                review_cell = (
                    f" | {REVIEW_STATUS_BADGE.get(issue.review_status, issue.review_status)}"
                )
            index_rows.append(
...
""" + "\n".join(index_rows) + "\n")
    ...
    for severity in [Severity.CRITICAL, Severity.HIGH, Severity.MEDIUM, Severity.LOW, Severity.INFO]:
        ...
        for issue in group:
            sections.append(_format_issue(issue, has_review))
    ...
    # ── Disclaimer ───────────────────────────────────────────────────────────
    review_note = (
        "\n\nThis report includes deterministic built-in review guidance. "
        "Review statuses and test recipes are advisory and require human verification."
        if has_review else ""
    )
    sections.append(f"""---
...
for a professional manual security audit.{review_note}
...
> These findings were detected but suppressed by the explicit documentation-trust option.
...
def _format_issue(issue: Issue, show_review: bool = False) -> str:
...
```"""
    ...
    review_block = ""
    if show_review and issue.review_status:
        status = REVIEW_STATUS_BADGE.get(issue.review_status, issue.review_status)
        review_block = f"""
**Built-in Deep Review** *({issue.review_engine})*
> **Status:** {status}
> {issue.review_reasoning or 'No review reasoning provided.'}

**Regression Test Recipe:**
{issue.review_test or 'Add a focused regression test for this finding.'}
"""
...
**Description:**
{issue.description}
{f"**Documentation Context (unverified):** {issue.context_note}" if issue.context_note else ""}
{snippet_block}
**Exploit Scenario:**
